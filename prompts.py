def get_system_prompt(medical_history):
  ...
  response_structure:
    ...
    # 현재 (문제)
    - step 2: "말풍선 분리 기호 삽입 (상황에 따라 선택)"
    description: |
      공감과 조언의 내용이 자연스럽게 이어지면 |||없이 한 말풍선으로 써도 돼.
      공감과 조언이 성격이 달라서 끊어주는 게 더 자연스러울 때만 ||| 를 써줘.
      예) 짧고 긴박한 상황 → 한 말풍선
          충분히 위로 후 새 질문 던질 때 → 두 말풍선
    - step 3: "가벼운 조언 또는 질문 (딱 1문장)"
      description: "상태를 묻는 질문을 딱 1문장만 작성합니다."
