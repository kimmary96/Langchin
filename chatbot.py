...
SYMPTOM_DEPARTMENT_MAP = {
    ...
    "잠": "정신건강의학과",
    "불면": "정신건강의학과",
    # 1. 여성 특화 질환 (3050 타겟이라면 반드시 필요합니다)
    "생리": "산부인과",
    "생리통": "산부인과",
    "질염": "산부인과",
    "자궁": "산부인과",
    "갱년기": "산부인과",
    "밑": "산부인과",  # 예: "밑이 빠질 것 같아", "밑이 간지러워"

    # 2. 방광 질환 (여성들이 피곤할 때 단골로 앓는 증상입니다)
    "소변": "비뇨의학과",  # 혹은 접근성을 위해 '내과'나 '산부인과'로 통일하셔도 좋습니다.
    "방광염": "비뇨의학과",
    "잔뇨": "비뇨의학과",

    # 3. 애매한 전신 증상 (어디 가야 할지 모를 때 만능인 곳입니다)
    "피로": "가정의학과",
    "몸살": "가정의학과",
    "무기력": "가정의학과",
    ...
    # 5. 일상 다빈도 표현 추가
    "체했": "내과",  # 소화 계열이지만 한국인은 "나 체했어"라는 말을 가장 많이 씁니다.
    "삐었": "정형외과",  # "발목 삐었어"
    "담": "정형외과",  # "목에 담 결렸어" (이 경우 한의원을 매핑하는 것도 3050 타겟에게 좋은 선택지입니다)
}
...
HOSPITAL_REQUEST_KEYWORDS = [
    "병원 추천", "병원 어디", "어디 병원", "어디가 좋을까", "병원 가야",
    "링크", "내과", "외과", "정형외과", "이비인후과",
    "피부과", "신경과", "추천해줘", "찾아줘",
]
...
class MomChatbot:
    ...
    def get_response(self, user_input, chat_history):
        ...
        if hospital_requested and department:
            extra_context = (
                f"\n\n[시스템 지시: 사용자가 병원을 직접 찾고 있습니다. 진료과({department}) 지도 링크 버튼은 시스템이 알아서 하단에 붙일 예정입니다. "
                f"당신은 '근처에 갈 만한 곳 찾아봤어. 늦지 않게 가보자'라는 식으로 다정하고 든든하게 병원 방문을 권유하는 멘트만 작성하세요.]"
            )
